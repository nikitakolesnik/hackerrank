...
bool compare_lists(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2) {
    ...
    while (true)
    {
        if (!curr1 && !curr2) // If the lists both end at the same time
            return true;
        if (!curr1 || !curr2) // If one list ends before the other
            return false;
        if (curr1->data != curr2->data) // If the values aren't equal
            return false;
